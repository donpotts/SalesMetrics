...
using AppSalesMetrics.Shared.Blazor.Models;
using AppSalesMetrics.Shared.Models;

namespace AppSalesMetrics.Shared.Blazor.Services;
    ...
    AuthenticationStateProvider authenticationStateProvider)
{
    ...
    public async Task<SalesMetric[]?> ListSalesMetricAsync()
    {
        var token = await authenticationStateProvider.GetBearerTokenAsync()
            ?? throw new Exception("Not authorized");

        HttpRequestMessage request = new(HttpMethod.Get, "/api/salesmetric");
        request.Headers.Authorization = new("Bearer", token);
        ...
        return await response.Content.ReadFromJsonAsync<SalesMetric[]>();
    }

    public Task<ODataResult<SalesMetric>?> ListSalesMetricODataAsync(
        int? top = null,
        int? skip = null,
        string? orderby = null,
        string? filter = null,
        ...
        string? expand = null)
    {
        return GetODataAsync<SalesMetric>("SalesMetric", top, skip, orderby, filter, count, expand);
    }

    public async Task<SalesMetric?> GetSalesMetricByIdAsync(long key)
    {
        var token = await authenticationStateProvider.GetBearerTokenAsync()
            ?? throw new Exception("Not authorized");

        HttpRequestMessage request = new(HttpMethod.Get, $"/api/salesmetric/{key}");
        request.Headers.Authorization = new("Bearer", token);
    ...
    public async Task UpdateSalesMetricAsync(long key, SalesMetric data)
    {
        var token = await authenticationStateProvider.GetBearerTokenAsync()
            ?? throw new Exception("Not authorized");

        HttpRequestMessage request = new(HttpMethod.Put, $"/api/salesmetric/{key}");
        request.Headers.Authorization = new("Bearer", token);
        request.Content = JsonContent.Create(data);
    ...
    public async Task<SalesMetric?> InsertSalesMetricAsync(SalesMetric data)
    {
        var token = await authenticationStateProvider.GetBearerTokenAsync()
            ?? throw new Exception("Not authorized");

        HttpRequestMessage request = new(HttpMethod.Post, "/api/salesmetric");
        request.Headers.Authorization = new("Bearer", token);
        request.Content = JsonContent.Create(data);
    ...
    public async Task DeleteSalesMetricAsync(long key)
    {
        var token = await authenticationStateProvider.GetBearerTokenAsync()
            ?? throw new Exception("Not authorized");

        HttpRequestMessage request = new(HttpMethod.Delete, $"/api/salesmetric/{key}");
        request.Headers.Authorization = new("Bearer", token);
    ...
    public async Task<OrderItem[]?> ListOrderItemAsync()
    {
        var token = await authenticationStateProvider.GetBearerTokenAsync()
            ?? throw new Exception("Not authorized");

        HttpRequestMessage request = new(HttpMethod.Get, "/api/orderitem");
        request.Headers.Authorization = new("Bearer", token);
    ...
    public Task<ODataResult<OrderItem>?> ListOrderItemODataAsync(
        int? top = null,
        int? skip = null,
        string? orderby = null,
        string? filter = null,
        ...
        string? expand = null)
    {
        return GetODataAsync<OrderItem>("OrderItem", top, skip, orderby, filter, count, expand);
    }

    public async Task<OrderItem?> GetOrderItemByIdAsync(long key)
    {
        var token = await authenticationStateProvider.GetBearerTokenAsync()
            ?? throw new Exception("Not authorized");

        HttpRequestMessage request = new(HttpMethod.Get, $"/api/orderitem/{key}");
        request.Headers.Authorization = new("Bearer", token);
    ...
    public async Task UpdateOrderItemAsync(long key, OrderItem data)
    {
        var token = await authenticationStateProvider.GetBearerTokenAsync()
            ?? throw new Exception("Not authorized");

        HttpRequestMessage request = new(HttpMethod.Put, $"/api/orderitem/{key}");
        request.Headers.Authorization = new("Bearer", token);
        request.Content = JsonContent.Create(data);
    ...
    public async Task<OrderItem?> InsertOrderItemAsync(OrderItem data)
    {
        var token = await authenticationStateProvider.GetBearerTokenAsync()
            ?? throw new Exception("Not authorized");

        HttpRequestMessage request = new(HttpMethod.Post, "/api/orderitem");
        request.Headers.Authorization = new("Bearer", token);
        request.Content = JsonContent.Create(data);
    ...
    public async Task DeleteOrderItemAsync(long key)
    {
        var token = await authenticationStateProvider.GetBearerTokenAsync()
            ?? throw new Exception("Not authorized");

        HttpRequestMessage request = new(HttpMethod.Delete, $"/api/orderitem/{key}");
        request.Headers.Authorization = new("Bearer", token);
    ...
    public async Task<Order[]?> ListOrderAsync()
    {
        ...
        HttpRequestMessage request = new(HttpMethod.Get, "/api/order");
        request.Headers.Authorization = new("Bearer", token);
    ...
    public async Task UpdateOrderAsync(long key, Order data)
    {
        var token = await authenticationStateProvider.GetBearerTokenAsync()
            ?? throw new Exception("Not authorized");

        HttpRequestMessage request = new(HttpMethod.Put, $"/api/order/{key}");
        request.Headers.Authorization = new("Bearer", token);
        request.Content = JsonContent.Create(data);
    ...
    public async Task<Order?> InsertOrderAsync(Order data)
    {
        var token = await authenticationStateProvider.GetBearerTokenAsync()
            ?? throw new Exception("Not authorized");
    ...
    public async Task DeleteOrderAsync(long key)
    {
        var token = await authenticationStateProvider.GetBearerTokenAsync()
            ?? throw new Exception("Not authorized");

        HttpRequestMessage request = new(HttpMethod.Delete, $"/api/order/{key}");
        request.Headers.Authorization = new("Bearer", token);
